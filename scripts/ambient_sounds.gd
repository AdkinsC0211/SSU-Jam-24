...
func _process(_delta: float) -> void:
	left_ear.global_transform.origin = global_transform.origin + Vector3(-1,0,0)
	right_ear.global_transform.origin = global_transform.origin + Vector3(1,0,0)
	
	left_gain_mod = 1
	if $LRayCast3D.is_colliding():
		left_gain_mod += ($LRayCast3D.get_collision_point() - $LRayCast3D.global_transform.origin).length() / 2
	if $LRayCast3D2.is_colliding():
		left_gain_mod += ($LRayCast3D.get_collision_point() - $LRayCast3D.global_transform.origin).length() / 2
	
	right_gain_mod = 1
	if $RRayCast3D.is_colliding():
		right_gain_mod += ($RRayCast3D.get_collision_point() - $RRayCast3D.global_transform.origin).length() / 2
	if $RRayCast3D2.is_colliding():
		right_gain_mod += ($RRayCast3D.get_collision_point() - $RRayCast3D.global_transform.origin).length() / 2
	
	audio_pan.pan = clampf(-1, 1, (check_length - right_gain_mod)/check_length - (check_length - left_gain_mod)/check_length)
